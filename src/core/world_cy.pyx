...
cdef class LinearDNA():
    ...
    cdef bint mergeable(self,LinearDNA dna):
        cdef int num_traits=5
        cdef double d1,d2,d3,d4,d5
        cdef double const=0.11523 #erfinv(1-0.5**(1/num_traits))
        cdef double p=1
...
cdef class PedigreeLLNode():
    def __cinit__(self,mid,fid,cid,moment):
        self.mid=mid
        self.fid=fid
        self.cid=cid
        self.moment=moment
        self.next=None
    def tuple(self):
        return self.mid,self.fid,self.cid,self.moment
cdef class PedigreeLL():
    def __cinit__(self):
        self.head=None
        self.tail=None
        self.current=None
        self.len=0
    cdef void append(self,PedigreeLLNode node):
        if self.len==0:
            self.head=node
            self.tail=node
            self.len=1
            ...
            self.tail=node
            self.len+=1
    cdef PedigreeLLNode next(self,PedigreeLLNode node):
        return node.next
    def __iter__(self):
        self.current=self.head
        return self
    ...
    cdef void childbirth(self,Minion girl,Minion boy):
        ...
        if girl.mergeable(boy):
            ...
            if self.record_pedigree:
                self.pedigree.append(PedigreeLLNode(girl.id,boy.id,child.id,self.moment))
