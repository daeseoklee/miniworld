...
#public API-------------------------------------------------------
def set_size(x,y):
    world_py.set_size(x,y)
def set_traits_range(ms1,ms2,ut1,ut2,ma1,ma2):
    world_py.set_traits_range(ms1,ms2,ut1,ut2,ma1,ma2)
def set_heredity(new_merge_thres,new_mut_per_diff):
    world_py.set_heredity(new_merge_thres, new_mut_per_diff)
def set_vision(new_vision_range,new_vision_resolution):
    world_py.set_vision(new_vision_range, new_vision_resolution)
def get_heredity():
    return world_py.merge_thres, world_py.mut_per_diff
def dna_with_opposite_color(dna):
    colorTrait,maxsizeTrait,uptakeTrait,maxageTrait,weightsTrait=dna.get_traits()
    return world_py.LinearDNA(colorTrait.opposite(),maxsizeTrait,uptakeTrait,maxageTrait,weightsTrait)
...
def construct_world(xsize,ysize,total_mass,num_dna,ininum,\
    ...
    dnas=[world_py.randLinearDNA() for _ in range(num_dna)]
    mis=[world_py.construct_minion(dnas[i%num_dna],\
    ...
    return world_py.World(xsize,ysize,total_mass,mis,no_age=no_age,no_birth=no_birth,\
